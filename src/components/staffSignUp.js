...
const SignUp = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        empId: Yup.number().typeError("Enter number").required('*required'),
        email: Yup.string().email("Enter valid email").required("*required"),
        phone: Yup.number().typeError("Enter a phone number").required('*required'),
        password: Yup.string().min(8, "Your password should be atleast 8 characters").max(8, 'Your password should atmost 8 characters').required('*required'),
                ...
                <Formik initialValues={initialValues} onSubmit={onSubmit} validationSchema={validationSchema}>
                    {(props) => (
                        <Form style={textStyle}>
                            ...
                            {/* Employee ID field */}
                            <Field as={TextField}
                                fullWidth name='empId'
                                variant="standard"
                                label='Employee ID'
                                required
                                placeholder="Enter your ID"
                                helperText={<ErrorMessage name="empId" render={(msg) => <span style={{ color: 'red' }}>{msg}</span>} />} />
